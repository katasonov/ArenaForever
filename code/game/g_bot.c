...
static void G_LoadArenasFromFile( char *filename ) {
	...
	if ( !f ) {
		g_trap_Print( va( S_COLOR_RED "file not found: %s\n", filename ) );
		return;
	}
	if ( len >= MAX_ARENAS_TEXT ) {
		g_trap_FS_FCloseFile( f );
		g_trap_Print( va( S_COLOR_RED "file too large: %s is %i, max allowed is %i\n", filename, len, MAX_ARENAS_TEXT ) );
		return;
	}
...
static void G_LoadArenas( void ) {
	...
	g_trap_Print( va( "%i arenas parsed\n", g_numArenas ) );
...
static void G_AddBot( const char *name, float skill, const char *team, int delay, char *altname) {
	...
	if (!*s ) {
		g_trap_Print( S_COLOR_RED "Error: bot has no aifile specified\n" );
		trap_BotFreeClient( clientNum );
		return;
	}
...
void Svcmd_AddBot_f( void ) {
	...
	if ( !name[0] ) {
		g_trap_Print( "Usage: Addbot <botname> [skill 1-5] [team] [msec delay] [altname]\n" );
		return;
	}
...
void Svcmd_BotList_f( void ) {
	...
	g_trap_Print("^1name             model            aifile              funname\n");
	for (i = 0; i < g_numBots; i++) {
		...
		g_trap_Print(va("%-16s %-16s %-20s %-20s\n", name, model, aifile, funname));
	}
}
...
char *G_GetBotInfoByNumber( int num ) {
	if( num < 0 || num >= g_numBots ) {
		g_trap_Print( va( S_COLOR_RED "Invalid bot number: %i\n", num ) );
		return NULL;
	}
