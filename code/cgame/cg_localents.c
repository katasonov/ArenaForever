...
void CG_FragmentBounceSound( localEntity_t *le, trace_t *trace ) {
	if ( le->leBounceSoundType == LEBS_BLOOD ) {
		...
		if ( rand() & 1 ) {
			...
			cg_trapS_StartSound( trace->endpos, ENTITYNUM_WORLD, CHAN_AUTO, s );
		}
...
void CG_AddFragment( localEntity_t *le ) {
	...
	if ( le->pos.trType == TR_STATIONARY ) {
		...
		if ( t < SINK_TIME ) {
			...
			cg_trapR_AddRefEntityToScene( &le->refEntity );
...
void CG_AddFadeRGB( localEntity_t *le ) {
	...
	cg_trapR_AddRefEntityToScene( re );
}
...
static void CG_AddExplosion( localEntity_t *ex ) {
	...
	if ( ex->light ) {
		...
		cg_trapR_AddLightToScene(ent->origin, light, ex->lightColor[0], ex->lightColor[1], ex->lightColor[2] );
	}
}
...
static void CG_AddSpriteExplosion( localEntity_t *le ) {
	...
	if ( le->light ) {
		...
		cg_trapR_AddLightToScene(re.origin, light, le->lightColor[0], le->lightColor[1], le->lightColor[2] );
	}
}
...
//			cg_trapS_StartSound (re->origin, ENTITYNUM_WORLD, CHAN_AUTO, cgs.media.kamikazeExplodeSound );
			cg_trapS_StartLocalSound(cgs.media.kamikazeExplodeSound, CHAN_AUTO);
		...
		cg_trapR_AddRefEntityToScene( &shockwave );
	}
...
//				cg_trapS_StartSound (re->origin, ENTITYNUM_WORLD, CHAN_AUTO, cgs.media.kamikazeImplodeSound );
				cg_trapS_StartLocalSound(cgs.media.kamikazeImplodeSound, CHAN_AUTO);
		...
		cg_trapR_AddRefEntityToScene( re );
		// add the dlight
		cg_trapR_AddLightToScene( re->origin, c * 1000.0, 1.0, 1.0, c );
	}
