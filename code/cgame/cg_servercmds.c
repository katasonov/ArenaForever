...
void CG_ParseServerinfo( void ) {
	...
	cg_trapCvar_Set("g_gametype", va("%i", cgs.gametype));
	...
	cg_trapCvar_Set("g_redTeam", cgs.redTeam);
	Q_strncpyz( cgs.blueTeam, Info_ValueForKey( info, "g_blueTeam" ), sizeof(cgs.blueTeam) );
	cg_trapCvar_Set("g_blueTeam", cgs.blueTeam);
}
...
static void CG_ParseWarmup( void ) {
	...
	if ( warmup == 0 && cg.warmup ) {
		...
		if (cgs.gametype >= GT_CTF && cgs.gametype <= GT_HARVESTER) {
			cg_trapS_StartLocalSound( cgs.media.countPrepareTeamSound, CHAN_ANNOUNCER );
		} else
#endif
		{
			cg_trapS_StartLocalSound( cgs.media.countPrepareSound, CHAN_ANNOUNCER );
		}
	}
	...
	while (o && *o) {
		...
		if (n && *n) {
			...
			if (o) {
				...
				timeOffset[o-t] = 0;
				o++;
				cg_trapR_RemapShader( originalShader, newShader, timeOffset );
			}
		} else {
			break;
		}
...
static void CG_ConfigStringModified( void ) {
	...
	if ( num == CS_MUSIC ) {
...
#ifdef MISSIONPACK
		cg_trapS_StartLocalSound( cgs.media.voteNow, CHAN_ANNOUNCER );
#endif //MISSIONPACK
		...
		cgs.gameModels[ num-CS_MODELS ] = cg_trapR_RegisterModel( str );
	} else if ( num >= CS_SOUNDS && num < CS_SOUNDS+MAX_SOUNDS ) {
		if ( str[0] != '*' ) {	// player specific sounds don't register here
			cgs.gameSounds[ num-CS_SOUNDS] = cg_trapS_RegisterSound( str, qfalse );
		}
...
static void CG_MapRestart( void ) {
	...
	if ( cg.warmup == 0 /* && cgs.gametype == GT_TOURNAMENT */) {
		cg_trapS_StartLocalSound( cgs.media.countFightSound, CHAN_ANNOUNCER );
		CG_CenterPrint( "FIGHT!", 120, GIANTCHAR_WIDTH*2 );
	}
#ifdef MISSIONPACK
	if (cg_singlePlayerActive.integer) {
		cg_trapCvar_Set("ui_matchStartTime", va("%i", cg.time));
		if (cg_recordSPDemo.integer && *cg_recordSPDemoName.string) {
			cg_trapSendConsoleCommand(va("set g_synchronousclients 1 ; record %s \n", cg_recordSPDemoName.string));
		}
	}
#endif
...
int CG_ParseVoiceChats( const char *filename, voiceChatList_t *voiceChatList, int maxVoiceChats ) {
	...
	if ( !f ) {
		cg_trapPrint( va( S_COLOR_RED "voice chat file not found: %s\n", filename ) );
		return qfalse;
	}
	if ( len >= MAX_VOICEFILESIZE ) {
		cg_trapPrint( va( S_COLOR_RED "voice chat file too large: %s is %i, max allowed is %i\n", filename, len, MAX_VOICEFILESIZE ) );
	...
	if (!Q_stricmp(token, "female")) {
		...
		cg_trapPrint( va( S_COLOR_RED "expected gender not found in voice chat file: %s\n", filename ) );
		return qfalse;
	}
	...
	while ( 1 ) {
		...
		token = COM_ParseExt(p, qtrue);
		if (Q_stricmp(token, "{")) {
			cg_trapPrint( va( S_COLOR_RED "expected { found %s in voice chat file: %s\n", token, filename ) );
			return qfalse;
		}
		...
		while(1) {
			...
			if (!Q_stricmp(token, "}"))
				break;
			sound = cg_trapS_RegisterSound( token, compress );
...
int CG_HeadModelVoiceChats( char *filename ) {
	...
	len = cg_trapFS_FOpenFile( filename, &f, FS_READ );
	if ( !f ) {
		//cg_trapPrint( va( "voice chat file not found: %s\n", filename ) );
		return -1;
	}
	if ( len >= MAX_VOICEFILESIZE ) {
		cg_trapPrint( va( S_COLOR_RED "voice chat file too large: %s is %i, max allowed is %i\n", filename, len, MAX_VOICEFILESIZE ) );
...
void CG_PlayVoiceChat( bufferedVoiceChat_t *vchat ) {
	...
	if ( !cg_noVoiceChats.integer ) {
		cg_trapS_StartLocalSound( vchat->snd, CHAN_VOICE);
...
static void CG_ServerCommand( void ) {
	...
	if ( !strcmp( cmd, "print" ) ) {
		...
		if ( !Q_stricmpn( cmd, "vote failed", 11 ) || !Q_stricmpn( cmd, "team vote failed", 16 )) {
			cg_trapS_StartLocalSound( cgs.media.voteFailed, CHAN_ANNOUNCER );
		} else if ( !Q_stricmpn( cmd, "vote passed", 11 ) || !Q_stricmpn( cmd, "team vote passed", 16 ) ) {
			cg_trapS_StartLocalSound( cgs.media.votePassed, CHAN_ANNOUNCER );
		}
#endif
		return;
	}
	...
	if ( Q_stricmp (cmd, "remapShader") == 0 )
	{
		if (cg_trapArgc() == 4)
		{
			...
			cg_trapR_RemapShader(shader1, shader2, shader3);
		}
...
void CG_ExecuteNewServerCommands( int latestSequence ) {
	while ( cgs.serverCommandSequence < latestSequence ) {
		if ( cg_trapGetServerCommand( ++cgs.serverCommandSequence ) ) {
			CG_ServerCommand();
		}
	}
}
