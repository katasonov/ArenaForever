...
void	cg_trap_R_AddLightToScene( const vec3_t org, float intensity, float r, float g, float b ) {
	syscall( CG_R_ADDLIGHTTOSCENE, org, CG_PASSFLOAT(intensity), CG_PASSFLOAT(r), CG_PASSFLOAT(g), CG_PASSFLOAT(b) );
}

void	cg_trap_R_AddAdditiveLightToScene( const vec3_t org, float intensity, float r, float g, float b ) {
	syscall( CG_R_ADDADDITIVELIGHTTOSCENE, org, CG_PASSFLOAT(intensity), CG_PASSFLOAT(r), CG_PASSFLOAT(g), CG_PASSFLOAT(b) );
}
...
void	cg_trap_R_DrawStretchPic( float x, float y, float w, float h, 
	...
	syscall( CG_R_DRAWSTRETCHPIC, CG_PASSFLOAT(x), CG_PASSFLOAT(y), CG_PASSFLOAT(w), CG_PASSFLOAT(h), CG_PASSFLOAT(s1), CG_PASSFLOAT(t1), CG_PASSFLOAT(s2), CG_PASSFLOAT(t2), hShader );
}
...
int		cg_trap_R_LerpTag( orientation_t *tag, clipHandle_t mod, int startFrame, int endFrame, 
	...
	return syscall( CG_R_LERPTAG, tag, mod, startFrame, endFrame, CG_PASSFLOAT(frac), tagName );
}
...
void		cg_trap_SetUserCmdValue( int stateValue, float sensitivityScale ) {
	syscall( CG_SETUSERCMDVALUE, stateValue, CG_PASSFLOAT(sensitivityScale) );
}
...
void		testPrintFloat( char *string, float f ) {
	syscall( CG_TESTPRINTFLOAT, string, CG_PASSFLOAT(f) );
}
