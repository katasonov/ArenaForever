...
public:
DownloadUpdatesCallback(HANDLE downloadFinishedEvent, std::function<bool(unsigned int downloaded, unsigned int total)> &&clbk)
...
private:
	...
	long		m_state;
	std::function<bool(unsigned int downloaded, unsigned int total)> _progressClbk;
};
